"""
Script para treinar um modelo Variational Autoencoder (VAE) em imagens médicas.
Inclui configuração via argumentos, carregamento de dados, inicialização do modelo, treinamento e salvamento de checkpoints.
"""
...
from util_training_vae import train_vae
...
if __name__ == '__main__':
    ...
    model = VarAutoEncoder(**config["stage1"]["params"]) # uma forma de upar o arquivo yaml pro wandb
    ...
    model, avg_train_losses, test_losses = train_vae(
        model=model,
        #l1_loss=l1_loss,
    ...
    model_save_path = os.path.join(output_dir, "vae")
    os.makedirs(model_save_path, exist_ok=True)
    torch.save(model.state_dict(), os.path.join(model_save_path, "vae_v0.pth"))
