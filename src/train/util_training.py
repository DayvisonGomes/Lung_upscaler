"""
Funções utilitárias para treinamento dos modelos AutoencoderKL, LDM e VQ-VAE.
Inclui loops de treino/validação, gerenciamento de otimizadores, early stopping, logging e métricas.
"""
...
def print_gpu_memory_report():
    if torch.cuda.is_available():
        ...
        print("Memory and GPU utilization report")
        for i in range(device_count):
            ...
            used_percent = int(used * 100.0 / total)
            
            # Utilização da GPU
            utilization = pynvml.nvmlDeviceGetUtilizationRates(handle)
            gpu_utilization = utilization.gpu  # Percentual de uso da GPU
            
            print(f"GPU {i}:")
            print(f"  Memória usada: {used_percent}%")
            print(f"  Utilização da GPU: {gpu_utilization}%")
...
def train_aekl(
    ...
    early_stop = EarlyStopping(patience=7, verbose=True, path=os.path.join(model_save_path, "model_aekl_best_4_latent_new_lung.pth"))
    ...
    torch.save(raw_model.state_dict(), os.path.join(model_save_path, "model_aekl_4_latent_new_lung.pth"))
...
def train_upsampler_ldm(
    ...
    early_stop = EarlyStopping(patience=21, verbose=True, path=os.path.join(model_save_path, "model_ldm_pulmao_Dsr_best.pth"))
    ...
    torch.save(raw_model.state_dict(), os.path.join(model_save_path, "model_ldm_pulmao_Dsr.pth"))
...
def train_epoch_upsampler_ldm(
    ...
    for step, x in pbar:
        ...
        mask_img = tifffile.imread(os.path.join(r'D:\Users\dayv\4d-lung-data\manifest-ObLxS9Wd1073675925233948759\data_lung_multiclass_masks', mask_path))
        ...
        mask_low_res_image = F.interpolate(mask_tensor, size=(256, 256), mode='bilinear', align_corners=False).to(device)
        ...
        with torch.amp.autocast('cuda', enabled=True):
            ...
            #mask_noise = torch.randn_like(mask_low_res_image).to(device)
            ...
            #noisy_mask_low_res_image = low_res_scheduler.add_noise(
            #    original_samples=mask_low_res_image, noise=mask_noise, timesteps=low_res_timesteps
            #)
            ...
            #noise_pred = model(x=latent_model_input, timesteps=timesteps)
            noise_pred = model(x=latent_model_input, timesteps=timesteps, class_labels=low_res_timesteps)
            
            alpha_t = scheduler.alphas_cumprod[timesteps.cpu()]
            k = 1
            gama = 0.5
            snr = alpha_t / (1 - alpha_t)
            loss_w = F.mse_loss(noise_pred.float(), noise.float()) / (k + snr) ** gama
        ...
        pbar.set_postfix(
            {
                ...
                "loss_w": f"{(epoch_loss_w / (step + 1)):.4f}"
            }
        )
    
    run.log({'mse_train_loss': epoch_loss / (quant_batch + 1)}, step=epoch)
...
def eval_upsampler_ldm(
    ...
    for x in loader:
        ...
        with torch.no_grad():
            with torch.amp.autocast('cuda', enabled=True):
                ...
                noise = torch.randn_like(latent).to(device)
                low_res_noise = torch.randn_like(low_res_image).to(device)
                #noise_mask = torch.randn_like(mask_low_res_image).to(device)
    ...
    latents = torch.randn((1, 4, 256, 256)).to(device)
    #gauss_noise = torch.randn((1, 1, 256, 256)).to(device)

    low_res_noise = torch.randn((1, 1, 256, 256)).to(device)
    noise_level = 1
    ...
    #mssim_value = mssim_metric(images, gauss_noise)
    ...
    run.log({
        ...
        #f"Generated Image": wandb.Image(gauss_noise[0, 0].cpu().numpy().astype(np.float32)),
