"""
Funções utilitárias para criação de dataloaders, transformações customizadas e manipulação de dados para experimentos de upscaling.
Inclui funções para carregar listas de imagens, aplicar transformações e preparar datasets.
"""
    ...
    def __call__(self, data):
        ...
        for key in self.key_iterator(d):
            ...
            if np.min(image_array) < 0:
                y = image_array * rescale_slope + rescale_intercept
            else:
                y = image_array * rescale_slope
            ...
            #d[key] = image_array.astype(np.float32)
            subname = file_path.split('\\')[-2]
            name = subname + '-' + file_path.split('\\')[-1].split('.')[0]
            d["filename"] = name
            #d['filename'] = file_path.split('\\')[-1]
            mask_path = name + '_mask.tiff'
            #mask_path = file_path.split('\\')[-1] + '_mask.tiff'
            #mask_img = tifffile.imread(os.path.join('D:\\Users\\dayv\\mamografIA_upscaler\\data\\data_lung_multiclass_masks', mask_path))
...
def get_upsampler_dataloader(batch_size: int,training_ids: str, validation_ids: str, num_workers: int = 8):
    ...
    train_datalist = get_datalist(ids_path=training_ids)[:1000]
    ...
    train_transforms = transforms.Compose(
        [   
            ...
            #Normalization(keys=["image"], min_val=0, max_val=2**16),
            Normalization_std(keys=["image"], mean=249.04623072382608, std=410.91504097992265),
            ...
            #transforms.Resized(keys=["image"],spatial_size=(low_res_size, low_res_size)),
