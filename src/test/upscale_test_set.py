"""
Executa o upscaling do conjunto de teste, calcula métricas de avaliação e salva imagens e logs.
Permite configuração detalhada via argumentos de linha de comando.
"""
...
if __name__ == '__main__':
    ...
    psnr_metric = PSNRMetric(max_val=5.0)
    ...
    for batch in tqdm(eval_loader, ncols=110):
        ...
        #output_resnet = output_resnet.view(output_resnet.size(0), 1, -1).to(image.device) #exp 2
        ...
        mask_low_res_image = F.interpolate(mask_tensor, size=(256, 256), mode='bilinear', align_corners=False).to(device) #exp4
        #mask_low_res_noise = torch.randn((mask_low_res_image.shape[0], 1, args.x_size, args.y_size)).to(device)
        ...
        for idx, image_batch in enumerate(batch['image']):
            ...
            img_upscale = new_images[-1][idx, 0].cpu().numpy()
            img = batch['image'][idx, 0].cpu().numpy()
            #mas_img_ = mask_low_res_image[idx, 0].cpu().numpy()
